<template>
  <div class="chineseStrokeTable" :style="{ gap: `${finalLineSpacing}px` }">
    <ChineseStrokeRow
...
import ChineseStrokeRow from '@/components/ChineseStrokeRow/ChineseStrokeRow.vue'
...
const handleOk = (args) => {
  ...
  for (let i = 0; i < rows.value.length; i++) {
    if (rows.value[i]?.word?.groupId === cell.groupId) {
      rows.value[i].word.pinyin = value
    }
  }
  emit('ok', args)
}
...
const rows = ref([])
watchEffect(async () => {
  const task = await Promise.allSettled(
    content.map((text) => getWriter(text, getValue(chineseStrokeSize))),
  )
  const data = content.map((word, idx) => {
    const info = pinyin(word, { type: 'all' })[0]
    const strokes = task[idx].value
    const id = uuidv4()
    return {
      word: word,
      info: info,
  ...
  const spaceNum = otherProps.unfilledNumber + unfilledRows + 1
  rows.value = origin.map((word, idx) => {
    return {
      ...otherProps,
      word: word || {},
